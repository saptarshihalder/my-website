...
function enterKey(e) {
  if (e.keyCode == 181) {
    ...
    if (e.keyCode == 13) {
      ...
      git = commands.length;
      addLine(
        "[saptarshi@archrx5500m]~$" + command.innerHTML,
        "no-animation",
        0
      );
...
function commander(cmd) {
  switch (cmd.toLowerCase()) {
    ...
    case "social":
      ...
      break;
      case "hack":
        hack.forEach((item, index) => {
          setTimeout(() => {
            addLine(item, "color2 margin", 1000 * index);
          }, index * 2000);
        });
        break;
    ...
    case "email":
      addLine(
        'Opening mailto:<a href="mailto:saptarshihalder69@gmail.com"> saptarshihalder69@gmail.com</a>...',
        "color2",
        80
      );
      newTab(email);
